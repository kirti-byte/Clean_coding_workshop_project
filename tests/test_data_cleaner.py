...
class TestDataCleaner(unittest.TestCase):
    ...
    def testFileExists(self):
        self.assertTrue(os.path.exists(cleaned_file_path), "Cleaned File exists")

    def testNullValues(self):
        cleaned_df = pd.read_csv(cleaned_file_path)
        is_null_val = cleaned_df.isnull().values.sum()
        self.assertGreater(is_null_val, 0, "Null values are present")

    def testColCount(self):
        cleaned_df = pd.read_csv(cleaned_file_path)
        num_cols = len(cleaned_df.axes[1])
        self.assertEqual(num_cols, 22)

    def testDateColumnType(self):
        cleaned_df = pd.read_csv(cleaned_file_path)
        columnToCheck = 'date'
        self.assertTrue(is_date_column(cleaned_df[columnToCheck]))

    def testColLen(self):
        cleaned_df = pd.read_csv(cleaned_file_path, dtype={'station_id': str, 'FRSHTT': str})
        col_name = 'FRSHTT'
        if col_name in cleaned_df.columns:
            len_of_col = cleaned_df[col_name].apply(lambda x: len(str(x)) == 6)
            self.assertTrue(all(len_of_col))
        else:
            self.fail(f"Column '{col_name}' not found in the DataFrame.")  
    ...
    def test_dataframe_schema(self):
        ...
        expected_dtypes = ['object', 'int64', 'object', 'float64', 'int64',
        ...
        for col, expected_dtype in zip(expected_columns, expected_dtypes):
            self.assertEqual(cleaned_df[col].dtype, expected_dtype)
